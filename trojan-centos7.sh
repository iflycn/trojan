...
if [ $real_addr == $local_addr ] ; then
	...
	wget https://raw.githubusercontent.com/iflycn/trojan/master/web.zip
    	unzip web.zip
	...
	tar xf trojan-1.*
	#下载trojan客户端
	wget https://raw.githubusercontent.com/iflycn/trojan/master/trojan-cli.zip
...
EOF
	...
	mkdir /usr/share/nginx/html/${trojan_passwd}
	mv /usr/src/trojan-cli/trojan-cli.zip /usr/share/nginx/html/${trojan_passwd}/
	#增加启动脚本
...
EOF
	...
	blue "http://${your_domain}/$trojan_passwd/trojan-cli.zip"
	green "2、将下载的压缩包解压，打开文件夹，运行start.bat即打开并运行Trojan客户端"
	green "3、运行stop.bat即关闭Trojan客户端"
